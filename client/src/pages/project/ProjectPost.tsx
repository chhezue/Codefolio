...
const ProjectPost: React.FC = () => {
  ...
  const dummyProjects: Record<string, ProjectDetails> = {
    ...
    "2": {
      ...
      projectDescription: '블록체인 기술을 활용한 안전하고 투명한 전자 투표 시스템',
      projectPeriod: '2022.11 - 2023.06',
      role: '백엔드 개발 및 블록체인 아키텍처 설계',
      technologies: ['Ethereum', 'Solidity', 'React', 'Node.js', 'Web3.js'],
      features: [
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Blockchain voting verification system with user interface&width=800&height=600',
          imageAlt: '블록체인 투표 검증',
          title: '블록체인 투표 검증',
          description: '투표 내역이 블록체인에 기록되어 변조가 불가능하며 투명하게 검증할 수 있습니다.'
        },
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Anonymous voting system with secure authentication&width=800&height=600',
          imageAlt: '익명 투표',
          title: '익명 투표 시스템',
          description: '영지식 증명을 활용한 익명 투표 시스템으로 투표자의 신원을 보호합니다.'
        }
      ],
      challenges: [
        {
          number: 1,
          title: '블록체인 확장성 문제',
          description: '대규모 투표에서의 블록체인 네트워크 부하 해결을 위한 레이어 2 솔루션 구현'
        },
        {
          number: 2,
          title: '신원 검증과 익명성',
          description: '신원 검증과 투표 익명성을 동시에 보장하기 위한 영지식 증명 프로토콜 도입'
        }
      ],
      screenshots: [
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Blockchain voting dashboard with statistics. Modern UI.&width=800&height=600',
          imageAlt: '투표 대시보드'
        },
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Voting verification process with blockchain explorer. Clean interface.&width=800&height=600',
          imageAlt: '투표 검증 화면'
        }
      ],
      docLinks: [
    ...
    "3": {
      ...
      projectDescription: '여러 사용자가 동시에 작업할 수 있는 실시간 마크다운 문서 편집 플랫폼',
      projectPeriod: '2023.03 - 2023.09',
      role: '프론트엔드 개발 및 실시간 동기화 로직 구현',
      technologies: ['React', 'TypeScript', 'Socket.io', 'Express', 'MongoDB'],
      features: [
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Real-time collaborative markdown editor with multiple cursors&width=800&height=600',
          imageAlt: '실시간 협업',
          title: '실시간 다중 사용자 협업',
          description: '여러 사용자가 동시에 문서를 편집하고 실시간으로 변경 사항을 확인할 수 있습니다.'
        },
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Markdown editor with live preview and formatting tools&width=800&height=600',
          imageAlt: '마크다운 미리보기',
          title: '실시간 마크다운 미리보기',
          description: '작성 중인 마크다운 문서의 렌더링 결과를 실시간으로 확인할 수 있습니다.'
        }
      ],
      challenges: [
        {
          number: 1,
          title: '동시 편집 충돌 해결',
          description: 'Operational Transformation 알고리즘을 구현하여 동시 편집 충돌 문제 해결'
        },
        {
          number: 2,
          title: '네트워크 지연 최적화',
          description: '네트워크 지연을 고려한 클라이언트 사이드 예측 및 서버 동기화 로직 개발'
        },
        {
          number: 3,
          title: '대규모 문서 성능 최적화',
          description: '대용량 문서 편집 시 발생하는 렌더링 성능 문제 해결을 위한 가상화 기법 적용'
        }
      ],
      screenshots: [
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Collaborative markdown editor with multiple cursors and user presence&width=800&height=600',
          imageAlt: '협업 에디터'
        },
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Markdown preview panel with formatting and styling options&width=800&height=600',
          imageAlt: '마크다운 미리보기'
        },
        {
          imageUrl: 'https://creatie.ai/ai/api/search-image?query=Document history and version control interface for markdown editor&width=800&height=600',
          imageAlt: '문서 버전 관리'
        }
      ],
      docLinks: [
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        setLoading(true);
        // ID에 해당하는 더미 데이터가 있는지 확인
        if (id && dummyProjects[id]) {
          setProjectData(dummyProjects[id]);
          setError(null);
          setLoading(false);
          return;
        }
        ...
        // 오류 발생 시 ID에 해당하는 더미 데이터가 있으면 사용
        if (id && dummyProjects[id]) {
          setProjectData(dummyProjects[id]);
          setError(null);
        } else {
          // 기본 데이터 설정 (첫 번째 더미 프로젝트)
          setProjectData(dummyProjects["1"]);
        }
      } finally {
        setLoading(false);
      }
